...
if ((global.controltype > 0) || (b1 > 0)) {
    ...
    draw_text_shadow(argument0 + xoff + w1 + b1 + sep, argument1 + yoff, p_text2);
    draw_sprite_ext(get_button_sprite(argument5), -1, argument0 + xoff + w1 + b1 + w2 + sep, argument1 + yoff + 6, 1, 1, 0, -1, argument6);
    draw_text_shadow(argument0 + xoff + w1 + b1 + w2 + b2 + sep * 2, argument1 + yoff, p_text3);
} else {
    if (argument3 == 0) draw_set_halign(fa_center);
    if (argument3 == 2) draw_set_halign(fa_right);
    draw_set_color(c_white); // og c_white
    draw_text_shadow(argument0 + xoff, argument1 + yoff, p_text1 + get_button_string(argument4) + p_text2 + get_button_string(argument5) + p_text3);
}
draw_set_alpha(1);
draw_set_halign(fa_left);
