...
oControl.mod_307 = scr_logic_SB_wall(true, false, false, true, true); // no gravity, no screw //AM2Retro//
...
//Need gravity suit && speedbooster && screwattack and atleast space-jump to get this item
while (true)
{
    oControl.mod_308 = ds_list_find_value(oControl.list_locations, ds_list_size(oControl.list_locations) - 1); // og mod_300
    if (oControl.mod_308 != 8 && oControl.mod_308 != 7 && oControl.mod_308 != 9)
    {
        if (oControl.mod_308 != 8) { break; }
        else { if (oControl.gravity_wall == false && oControl.screw_wall == false) { break; } }
    } 
    ds_list_shuffle(oControl.list_locations);
}
ds_list_delete(oControl.list_locations, ds_list_size(oControl.list_locations) - 1);
...
oControl.mod_300 = scr_random_draw_pool(); // added AM2Retro
