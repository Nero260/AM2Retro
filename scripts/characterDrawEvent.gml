...
//Old Speedbooster Colors.
if (speedboost == 1 || charge > 0 || state == SJSTART || state == SUPERJUMP || state == SJEND && statetime < 10) {
    if (fxtimer == 0 || fxtimer == 1) draw_character_from_surface(x, y, c_blue, 1.1 + fxtimer * 0.2); //c_green
    if (fxtimer == 2 || fxtimer == 3) {
        draw_character_from_surface(x, y, c_orange, 1); //c_yellow
    }
}
...
//draw_set_blend_mode(bm_add);
//draw_character_from_surface(x, y, c_white, 1); //c_white
        //draw_character_from_surface(x, y, c_white, 1); //c_white
        //draw_set_blend_mode(bm_normal);

    /*if (fxtimer == 4 || fxtimer == 5) {
        //draw_set_blend_mode(bm_add);
        draw_character_from_surface(x, y, c_red, 1); //c_red
        //draw_set_blend_mode(bm_normal);
    }*/
