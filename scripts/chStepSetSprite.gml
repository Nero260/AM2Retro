...
if (oControl.mod_sprintanimation == 1) {
    if (state == RUNNING && speedboost) { // && speedboost) {
        if (facing == RIGHT && walking == 0) {  // walking == 0) {
            if (aimdirection == 0 && firing == 0 && armmsl == 0) {
                if (set == 0) sprite_index = scr_suit_sprites(sSpdBstR,sSpdBstR_fusion);
                if (set == 1) sprite_index = scr_suit_sprites(sVSpdBstR,sSpdBstR_fusion);
                if (set == 2) sprite_index = scr_suit_sprites(sGSpdBstR,sSpdBstR_fusion);              
                image_speed = 0.6;
            }
            if (aimdirection == 0 && firing > 0 || armmsl == 1) {
                if (set == 0) sprite_index = scr_suit_sprites(sRunAimR,sRunAimR_fusion);
        ...
        if (facing == LEFT && walking == 0) {  // walking == 0) {
            if (aimdirection == 1 && firing == 0 && armmsl == 0) {
                if (set == 0) sprite_index = scr_suit_sprites(sSpdBstL,sSpdBstL_fusion);
                if (set == 1) sprite_index = scr_suit_sprites(sVSpdBstL,sSpdBstL_fusion);
                if (set == 2) sprite_index = scr_suit_sprites(sGSpdBstL,sSpdBstL_fusion);
                image_speed = 0.6;
            }
            if (aimdirection == 1 && firing > 0 || armmsl == 1) {
                if (set == 0) sprite_index = scr_suit_sprites(sRunAimL,sRunAimL_fusion);
...
if (global.screwattack == 1 && state == JUMPING && vjump == 0 && walljumping == 0 && (inwater == 0 || global.currentsuit >= 2)) {
    if (fxtimer == 0) image_blend = make_color_rgb(120, 0, 160); // make_color_rgb(120, 0, 160);
    if (fxtimer == 2) image_blend = make_color_rgb(150, 200, 150); // make_color_rgb(150, 200, 150);
    if (fxtimer == 4) image_blend = make_color_rgb(180, 255, 200); // make_color_rgb(180, 255, 200);
    /*if (fxtimer == 0) image_blend = make_color_rgb(150, 0, 200);
    if (fxtimer == 2) image_blend = make_color_rgb(200, 90, 150);
    if (fxtimer == 4) image_blend = make_color_rgb(255, 180, 255);*/
    ...
    //mytrail.image_blend = c_blue;
    mytrail.image_index = image_index;
    //mytrail.image_blend = c_red;
    ...
    //mytrail.image_blend = c_blue;
    mytrail.image_blend = make_color_rgb(50 + random(200), 255, 50 + random(200)); // mytrail.image_blend = make_color_rgb(50 + random(200), 255, 50 + random(200));
    //mytrail.image_blend = make_color_rgb(255, 50 + random(200),255);
...
if (state == JUMPING && global.spacejump == 1 && vjump == 0 && dash == 0 && charge == 0 && speedboost == 0 && (inwater == 0 || global.currentsuit >= 2)) {
    ...
    if (facing == LEFT) mytrail.image_xscale = -1;
    //if (global.screwattack = 1) mytrail.image_blend = c_fuchsia;
